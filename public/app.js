...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function agregarServicioInicialEnCategorias() {
    ...
    if (
      !document.querySelector("#serviciosCategoriasContainer .servicio-item")
    ) {
...
async function guardarPaciente() {
  ...
  try {
    ...
    const response = await fetch("/api/pacientes", {
      ...
      body: JSON.stringify({
        nombre: nombre,
        correo: correo || null, // Enviar null si está vacío
    ...
    // 3. Actualizar la lista de pacientes
    await cargarPacientes();

    // 4. Seleccionar automáticamente el nuevo paciente
    if ($("#pacienteSelect").length && $("#pacienteSelect").data("select2")) {
      $("#pacienteSelect").val(nuevoPaciente.id).trigger("change");
    } else {
      document.getElementById("pacienteSelect").value = nuevoPaciente.id;
    }
      ...
      $("#pacienteSelect").select2({
        ...
        language: {
          ...
          noResults: function (params) {
            if (params && params.term && params.term.length > 0) {
              return (
                "Presiona Enter para usar: <strong>" + params.term + "</strong>"
              );
            }
        ...
        createTag: function (params) {
          ...
          $("#pacienteSelect option").each(function () {
            if ($(this).text().toLowerCase() === params.term.toLowerCase()) {
              exists = true;
              return false;
            }
          });
        ...
        templateResult: function (data) {
          if (data.newOption) {
            return $('<span><em>Escribir: "' + data.text + '"</em></span>');
          }
          return data.text;
        },
...
function toggleNuevoPacienteForm() {
  ...
  if (form.style.display === "none" || !form.style.display) {
    ...
    pacienteSelect.value = ""; // Limpiar selección actual
    if ($("#pacienteSelect").data("select2")) {
...
// En setupEventListeners, asegurar este evento:
document
...
function setupPacienteSearchBar() {
  ...
  // Si no existen los elementos críticos, salir silenciosamente
  if (!input || !results) return;

  // Resto de la implementación de la barra de búsqueda...
  let currentResults = [];
  let selectedIndex = -1;
