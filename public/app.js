...
document.addEventListener("DOMContentLoaded", function () {
  ...
  async function guardarCotizacion(e) {
    ...
    fases.forEach((f) =>
      alert(
        `Fase ${f.numero_fase} | dur=${f.duracion} | cats=[${f.categorias.join(
          ", "
        )}] | obs="${f.observacion}"`
      )
    );
    ...
    try {
      ...
      console.log("[Cotización] %s %s", method, url, cotizacionSimplificada);
    ...
    for (const fase of fases) {
      try {
        ...
        console.log(`[Fase] OK fase ${fase.numero_fase} => id=${faseId}`);
        alert(`Fase ${fase.numero_fase} guardada (id=${faseId ?? "?"})`);

        // Guardar FaseCategorias (una por cada categoría detectada)
        if (Array.isArray(fase.categorias) && fase.categorias.length > 0) {
          const catOps = fase.categorias.map(async (categoriaId) => {
            ...
            if (!r.ok) {
              throw new Error(`fase=${faseId} cat=${categoriaId} => ${txt}`);
            }
          ...
          const rechazadas = settled.filter((r) => r.status === "rejected");
