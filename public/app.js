    ...
    try {
      ...
      if (categorias.length > 0) {
        console.log("Primera categoría:", {
          id: categorias[0].id,
          nombre: categorias[0].nombre_categoria,
        });
      }
    } catch (error) {
...
`;
  ...
  function agregarFase() {
    try {
      // Calcular el próximo número de fase
      const fasesExistentes = document.querySelectorAll(".fase-card");
      const nextFaseNum = fasesExistentes.length + 1;

      const template = document.getElementById("faseTemplate");
      const clone = template.content.cloneNode(true);
      const faseCard = clone.querySelector(".fase-card");
      faseCard.dataset.faseId = nextFaseNum;

      // Configurar número de fase
      const faseNumero = faseCard.querySelector(".fase-numero");
      faseNumero.textContent = nextFaseNum;
      ...
      // Agregar primer servicio automáticamente
      agregarServicioAFase(faseCard);
    } catch (error) {
      console.error("Error al agregar fase:", error);
      alert(`Error al agregar fase: ${error.message}`);
    }
  }

  // Función para reordenar las fases después de eliminar una
  function reordenarFases() {
    const fases = document.querySelectorAll(".fase-card");
    fases.forEach((fase, index) => {
      const newNum = index + 1;
      fase.dataset.faseId = newNum;
      const faseNumero = fase.querySelector(".fase-numero");
      if (faseNumero) {
        faseNumero.textContent = newNum;
      }
    });
  }
  ...
  function agregarServicioAFase(faseCard) {
    try {
      // 1. Obtener plantilla y clonar
      const template = document.getElementById("servicioTemplate");
      if (!template) throw new Error("Plantilla de servicio no encontrada");

      const clone = template.content.cloneNode(true);
      const servicioItem = clone.querySelector(".servicio-item");
      if (!servicioItem) throw new Error("Estructura de servicio incorrecta");

      // 2. Configurar elementos principales
      ...
      const precioUnitario = servicioItem.querySelector(".precio-unitario");
      const cantidadInput = servicioItem.querySelector(".cantidad");
      const descuentoInput = servicioItem.querySelector(".descuento");
      const totalServicio = servicioItem.querySelector(".total-servicio");
      ...
      categorias.forEach((categoria) => {
        // Formatear el ID con ceros a la izquierda (ej. 5 → "05")
        const idFormateado = String(categoria.id).padStart(2, "0");

        // Crear texto mostrado en el dropdown
        const textoMostrado = `${idFormateado} - ${
          categoria.nombre_categoria
        }: ${categoria.descripcion || "Sin descripción"}`;

        const option = new Option(
          textoMostrado, // Texto mostrado
          categoria.id, // Valor real
          false, // selected?
          false // disabled?
        );

        // Guardar datos originales para usarlos en los templates
        option.dataset.nombre = categoria.nombre_categoria;
        option.dataset.descripcion = categoria.descripcion || "";

        categoriaSelect.add(option);
      });

      // 5. Configuración mejorada de Select2
      $(categoriaSelect).select2({
        width: "100%",
        placeholder: "Seleccionar categoría",
        dropdownParent: faseCard,
        templateResult: formatCategoriaDisplay, // Función para mostrar en dropdown
        templateSelection: formatCategoriaSelection, // Función para mostrar seleccionado
      });

      // 6. Configuración del dropdown de servicios
      $(servicioSelect).select2({
        width: "100%",
        placeholder: "Seleccionar servicio",
        disabled: true,
        dropdownParent: faseCard,
        templateResult: function (servicio) {
          if (!servicio.id) return servicio.text;
          ...
          // Parte 1: Código y Nombre
          const $linea1 = $("<div>").append(
            $("<strong>").text(servicio.element.dataset.codigo + " - "),
            $("<span>").text(servicio.element.dataset.nombre)
          );

          // Parte 2: Descripción (si existe)
          if (servicio.element.dataset.descripcion) {
            const $linea2 = $("<div>")
              .css({
                color: "#666",
                "font-size": "0.9em",
              ...
              .text(servicio.element.dataset.descripcion);

            $container.append($linea1, $linea2);
          } else {
            $container.append($linea1);
          }

          return $container;
        },
        templateSelection: function (servicio) {
          if (!servicio.id) return servicio.text;
          return $("<span>").text(
            `${servicio.element.dataset.codigo} - ${servicio.element.dataset.nombre}`
          );
        },
      });

      // 7. Evento cambio de categoría (actualizado)
      $(categoriaSelect).on("change", function (e) {
        const categoriaId = $(this).val();

        // Limpiar y deshabilitar servicio primero
        ...
        if (categoriaId) {
          ...
          // Llenar servicios con formato mejorado
          serviciosFiltrados.forEach((servicio) => {
            ...
            // Almacenar datos para Select2
            option.dataset.codigo = servicio.codigo;
            option.dataset.nombre = servicio.descripcion;
          ...
          $(servicioSelect).prop("disabled", false).trigger("change");
        }
      });

      // 8. Evento cambio de servicio (actualizado)
      ...
      // 9. Función para calcular total del servicio
      function calcularServicio() {
        ...
        // Formatear el total para eliminar .00 si es entero
        servicioItem.querySelector(".total-servicio").value =
          formatPrice(total);
      ...
      function formatPrice(amount) {
        // Si es un número entero, mostrar sin decimales, de lo contrario mostrar 2 decimales
        return amount % 1 === 0 ? amount.toString() : amount.toFixed(2);
      }

      // 10. Eventos para inputs
      cantidadInput.addEventListener("input", calcularServicio);
      descuentoInput.addEventListener("input", calcularServicio);

      // 11. Evento eliminar servicio
      servicioItem
  ...
  // Función para formatear cómo se ven las opciones en el dropdown
  function formatCategoriaDisplay(categoria) {
    if (!categoria.id) return categoria.text;

    const $container = $("<span>");
    const idPart = $("<span>")
      .text(categoria.id.padStart(2, "0") + " - ")
      .css("font-weight", "bold");
    const nombrePart = $("<span>").text(categoria.element.dataset.nombre);

    $container.append(idPart).append(nombrePart);

    // Añadir descripción si existe
    if (categoria.element.dataset.descripcion) {
      $container.append(
        $("<span>")
          .text(": " + categoria.element.dataset.descripcion)
          .css("color", "#666")
          .css("font-size", "0.9em")
      );
    }
  ...
  // Función para formatear cómo se ve la selección
  function formatCategoriaSelection(categoria) {
    if (!categoria.id) return categoria.text;

    // Mostrar solo ID y nombre cuando está seleccionado
    return $("<span>").text(
      `${String(categoria.id).padStart(2, "0")} - ${
        categoria.element.dataset.nombre
      }`
    );
  }

  // Función auxiliar para cálculos
  function calcularServicio(servicioItem, faseCard) {
    const precio =
      parseFloat(servicioItem.querySelector(".precio-unitario").value) || 0;
    const cantidad =
      parseInt(servicioItem.querySelector(".cantidad").value) || 1;
  ...
  function calcularTotalesFase(faseCard) {
    ...
    // Función auxiliar para formatear sin .00 cuando es entero
    const formatPrice = (amount) => {
      return amount % 1 === 0 ? amount.toString() : amount.toFixed(2);
    };

    faseCard.querySelector(".fase-subtotal").textContent = `$${formatPrice(
      subtotal
    )}`;
    faseCard.querySelector(".fase-descuento").textContent = `$${formatPrice(
      descuentoTotal
    )}`;
    faseCard.querySelector(".fase-total-amount").textContent = `$${formatPrice(
      total
    )}`;
  }
  ...
  function calcularTotalesGenerales() {
    ...
    // Función para formatear eliminando .00 en números enteros
    const formatPrice = (amount) => {
      return amount % 1 === 0 ? amount.toString() : amount.toFixed(2);
    };
    ...
    ).textContent = `$${formatPrice(descuentoTotal)}`;
    document.getElementById("total-cotizacion").textContent = `$${formatPrice(
      total
    )}`;
  }
