...
function addNewService(serviceList) {
    ...
    if (!template) {
        console.error(
            "No se encontró el template de servicio (service-template)"
        );
        alert(
            "Error: No se encontró el template de servicio. Verifica el HTML."
        );
        return;
    }
    const newService = template.content.cloneNode(true);
    // Set up remove button
    const removeBtn = newService.querySelector(".remove-servicio");
    if (removeBtn) {
        ...
        console.error(
            "No se encontró el botón .remove-servicio en el template de servicio"
        );
    }
    // Set up add button
    const addBtn = newService.querySelector("add-service-btn");
    if (addBtn) {
        ...
        console.error(
            "No se encontró el botón .agregar-servicio en el template de servicio"
        );
    }
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function inicializarPrimeraCategoria() {
        ...
        const serviciosContainer = document.querySelector(
            "#no-phase-categories .category-actions"
        );
        ...
        if (categoriaSelect) {
            ...
            //console.log(addServiceBtn);
            console.log(categoriaSelect);
            console.log(
                "Valor actual de categoriaSelect:",
                categoriaSelect ? categoriaSelect.value : "(no definido)"
            ); //console.log(serviciosContainer);
            if (addServiceBtn) {
