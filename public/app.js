...
function addNewPhase() {
    ...
    const phaseCount =
        (phasesContainer.querySelectorAll(".phase-container:not(.hidden)")
            .length || 0) + 1;
...
function addNewService(serviceList) {
    ...
    const categorySelect = serviceList.closest(".category-group")?.querySelector(".categoria-fase-select, .categoria-unica-select");
    ...
    if (categorySelect && servicioSelect) {
        ...
        if (categorySelect.value) {
            const serviciosCategoria = servicios.filter((s) => String(s.categoria_id) === String(categorySelect.value));
        ...
        if (!categorySelect._listenerServicios) {
            categorySelect.addEventListener("change", function () {
                servicioSelect.innerHTML = '<option value="">Seleccionar servicio...</option>';
                if (this.value) {
                    const serviciosCategoria = servicios.filter((s) => String(s.categoria_id) === String(this.value));
                    serviciosCategoria.forEach((servicio) => {
                        const option = document.createElement("option");
                        option.value = servicio.id;
                        option.textContent = `${servicio.codigo} - ${servicio.descripcion}`;
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function inicializarPrimeraCategoria() {
        ...
        const serviciosContainer = document.querySelector("#no-phase-categories .servicios-categorias-container");
        if (categoriaSelect) {
            ...
            const addServiceBtn = document.querySelector("#no-phase-categories .add-service-btn");
            if (addServiceBtn) {
                addServiceBtn.addEventListener("click", function (e) {
                    e.preventDefault();
                    if (categoriaSelect.value && serviciosContainer) {
                        agregarServicioEnCategoriasDinamico(serviciosContainer, categoriaSelect.value);
                    }
                });
            }
    ...
    if (servicioSelect) {
        servicioSelect.innerHTML = '<option value="">Seleccionar servicio...</option>';
        const serviciosCategoria = servicios.filter((s) => String(s.categoria_id) === String(categoriaId));
        serviciosCategoria.forEach((servicio) => {
            const option = document.createElement("option");
            option.value = servicio.id;
            option.textContent = `${servicio.codigo} - ${servicio.descripcion}`;
        ...
        // No seleccionar automáticamente el primer servicio, dejar vacío
    }
